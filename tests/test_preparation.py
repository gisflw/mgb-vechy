...
def _spec(tmp_path, *, return_stage_reports=False):
    ...
    segments = VectorTable.from_pydict(
        {"id": [1], "down": [None], "order": [1], "up": [1.0], "length": [1.0]},
    ...
    aggregation_report = aggregate_roi_dataset(
        AggregationSpec(
            roi=tmp_path / "roi",
            uparea_min=0,
            lmin=0,
...
def test_vector_stage_cli_prints_timings(tmp_path):
    ...
    roi_result = runner.invoke(
        ...
        [
            ...
            "EPSG:3857",
            "--catchments",
            str(tmp_path / "catchments.fgb"),
            "--segments",
            str(tmp_path / "segments.fgb"),
            "--outlet-id",
            "1",
            "--id-col",
            "id",
    ...
    aggregation_result = runner.invoke(
        main,
        [
            "aggregate",
            "--roi",
...
def test_prepare_rejects_obsolete_aggregation_contract(tmp_path):
    spec = _spec(tmp_path)
    manifest_path = spec.minis / "manifest.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["version"] = 1
    manifest_path.write_text(json.dumps(manifest))

    with pytest.raises(PreparedDataError, match="Unsupported aggregation"):
